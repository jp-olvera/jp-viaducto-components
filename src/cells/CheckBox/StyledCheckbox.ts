...
export const StyledLabel = styled.label<any>`
  ...
  ${({ disabled, configuration }) =>
    disabled ? `color: ${configuration.colors.disableColor}` : null};
  ...
  & input {
    ...
    ${({ inputSize }) => getCheckSizes(inputSize).mark};
    ...
    &:hover:not(:disabled) {
      ${({ inputSize }) => getCheckSizes(inputSize).mark_size};
      background-color: ${({ color, configuration }) =>
        configuration.colors[color].default || color || configuration.colors.primary.default};
      opacity: 0.8;
    }

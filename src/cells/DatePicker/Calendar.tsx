...
import { dateBetweenSelected, isToday, onClickDay, setClassName } from './datePickerFunctions';
  ...
  return (
    <div className='calendar' {...rest}>
      <DatePickerHeader
        ...
        months={months}
      />
      <Container vertical='xs' horizontal='xs' expandHorizontal className='date-calendar'>
        <WeekDays days={days} />
      </Container>
      <Container vertical='xs' horizontal='xs' expandHorizontal className='date-calendar'>
          ...
          <Day
            ...
            disabled={
              !!(
                range &&
                firstDate &&
                dateCompare(firstDate, new Date(today.getFullYear(), today.getMonth(), data))
              )
            }
            onClick={() =>
              onClickDay(
            ...
            className={`${
              ((selected.selectedDate?.getDate() || -50) + 1 === data + 1 && range === false) ||
              (range === true && dateBetweenSelected(isBetween, firstDate, today, data)) ||
              setClassName(today, firstDate, selected.prevSelectedDate, data + 1) ||

...
import { render, screen, axe, fireEvent } from '../../../test-utils';
import { Pill } from '..';

describe('<Pill/>', () => {
  test('should render properly', () => {
    render(<Pill label='Pill' iconLead='' icon='' borderColor='red' circleBorder={false} />);
    expect(screen.queryByText('Pill')).toBeVisible();
  });
  test('should render properly with default props', () => {
    render(<Pill label='Pill' circleBorder radius='sm' />);
    expect(screen.queryByText('Pill')).toBeVisible();
  });
  ...
  test('should call handleAction', async () => {
    const click = jest.fn();
    const { container, getByRole } = render(<Pill label='' handleAction={click} />);
